...
# Pass strategies via env vars; your project Makefile / Tcl reads these.
...
gsutil -m cp -r out/*.bit "gs://BUCKET_PLACEHOLDER/JOB_NAME_PLACEHOLDER/${TAG}/" 2>/dev/null || true

# Extract WNS (Worst Negative Slack) from the timing report and stash it
WNS=$(grep -Po 'WNS\(ns\)\s*\K-?[\d.]+' out/*.rpt | head -1 || echo "N/A")
